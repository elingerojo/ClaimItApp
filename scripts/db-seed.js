...
async function main() {
  ...
  try {
    ...
    // 5. Items: copy the 5 real seed items, reuse image_urls (JSONB array),
    //    store only the base cost (role prices are computed at read time).
    for (const it of SEED_ITEMS) {
      await client.query(
        `INSERT INTO items
          (title, description, category, info_url, image_urls, status, visibility_level,
           event_id, visible_at, available_from,
           precio_base_costo,
           nivel_acceso_minimo)
